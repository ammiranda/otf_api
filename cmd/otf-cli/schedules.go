...
var schedulesCmd = &cobra.Command{
	Use:   "schedules",
	Short: "Browse and book classes",
	Long: `Fetches class schedules and lets you browse/book interactively.

Interactive mode shows color-coded classes grouped by day. Select one
to see details and book it (or join the waitlist if full).

With --json, outputs the raw schedule.
With --class-id, --book, and --yes, books a class non-interactively.

Examples:
  otf-cli schedules                              # interactive browse & book
  otf-cli schedules --json                       # output as JSON
  otf-cli schedules --class-id "<id>" --book     # book with confirmation
  otf-cli schedules --class-id "<id>" --book --yes  # book silently`,
